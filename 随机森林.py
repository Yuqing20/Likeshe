...
for train_index, test_index in kf.split(X_train_c, y_train_c):
    ...
    # 使用随机森林进行训练
    # 创建分类器对象
    model = RandomForestClassifier(max_depth=3, n_estimators=500, min_samples_leaf=10, random_state=123)
...
# 将结果写到csv文件里
df.to_csv("baseline 随机森林.csv", index=False)
